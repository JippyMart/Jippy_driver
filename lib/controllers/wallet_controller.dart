...
class WalletController extends GetxController {
  ...
  RxString selectedPaymentMethod = "razorpay".obs; // Default to Razorpay
  ...
  @override
  void onInit() {
    AppLogger.log('WalletController onInit() called', tag: 'Controller');
  ...
  getPaymentSettings() async {
    await FireStoreUtils.getPaymentSettingsData().then(
      (value) {
        ...
        // Stripe initialization commented out - using Razorpay instead
  ...
  walletTopUp() async {
    ...
    print('💰 [Wallet Controller] Amount: ${topUpAmountController.value.text}');
    
    // Temporarily disable integrity check for wallet top-up in production
    // TODO: Re-enable after fixing Play Integrity issues
    print('💰 [Wallet Controller] Integrity check temporarily disabled for wallet top-up');
    
    // Comment out integrity check for now
    /*
    print('💰 [Wallet Controller] Verifying device integrity before wallet top-up...');
    // Verify device integrity before wallet top-up
    final isIntegrityVerified = await PlayIntegrityUtils.verifyBeforeWallet();
    print('💰 [Wallet Controller] Integrity verification result: $isIntegrityVerified');
    PlayIntegrityUtils.logIntegrityCheck('Wallet Top-up', isIntegrityVerified);
    
    if (!isIntegrityVerified) {
      print('💰 [Wallet Controller] ❌ Wallet top-up blocked due to integrity check failure');
...
//Paypal
  ...
  // Stripe payment methods commented out - using Razorpay instead
  // Future<void> stripeMakePayment({required String amount}) async {
  //   print('💳 [Wallet Controller] Starting Stripe payment for amount: $amount');
  //   log(double.parse(amount).toStringAsFixed(0));
  //   
  //   print('💳 [Wallet Controller] Verifying device integrity before Stripe payment...');
  //   // Verify device integrity before payment
  //   final isIntegrityVerified = await PlayIntegrityUtils.verifyBeforePayment();
  //   print('💳 [Wallet Controller] Integrity verification result: $isIntegrityVerified');
  //   PlayIntegrityUtils.logIntegrityCheck('Stripe Payment', isIntegrityVerified);
  //   
  //   if (!isIntegrityVerified) {
  //     print('💳 [Wallet Controller] ❌ Payment blocked due to integrity check failure');
  //     ShowToastDialog.showToast("Security check failed. Please try again.".tr);
  //     return;
  //   }
  //   
  //   print('💳 [Wallet Controller] ✅ Integrity check passed, proceeding with payment...');
  //   try {
  ...
  void openCheckout({required amount, required orderId}) async {
    print('💳 [Wallet Controller] Starting Razorpay payment for amount: $amount, orderId: $orderId');
    
    // Temporarily disable integrity check for Razorpay payments
    // TODO: Re-enable after fixing Play Integrity issues
    print('💳 [Wallet Controller] Integrity check temporarily disabled for Razorpay payments');
    
    // Comment out integrity check for now
    /*
    print('💳 [Wallet Controller] Verifying device integrity before Razorpay payment...');
    // Verify device integrity before payment
    final isIntegrityVerified = await PlayIntegrityUtils.verifyBeforePayment();
    print('💳 [Wallet Controller] Integrity verification result: $isIntegrityVerified');
    ...
    if (!isIntegrityVerified) {
      print('💳 [Wallet Controller] ❌ Razorpay payment blocked due to integrity check failure');
      ShowToastDialog.showToast("Security check failed. Please try again.".tr);
      return;
    }
    */
    
    print('💳 [Wallet Controller] ✅ Proceeding with Razorpay payment...');
    ...
    try {
      print('💳 [Wallet Controller] Opening Razorpay checkout with options...');
      razorPay.open(options);
      print('💳 [Wallet Controller] ✅ Razorpay checkout opened successfully');
    } catch (e) {
      print('💳 [Wallet Controller] ❌ Razorpay checkout failed');
      print('💳 [Wallet Controller] Error: $e');
      ShowToastDialog.showToast("Payment initialization failed. Please try again.".tr);
    }
  }

  void handlePaymentSuccess(PaymentSuccessResponse response) {
    print('💳 [Wallet Controller] Razorpay payment success received');
    print('💳 [Wallet Controller] Payment ID: ${response.paymentId}');
    print('💳 [Wallet Controller] Order ID: ${response.orderId}');
    print('💳 [Wallet Controller] Signature: ${response.signature}');
    
    print('💳 [Wallet Controller] Verifying device integrity before processing payment success...');
    // Verify device integrity before processing payment success
    PlayIntegrityUtils.verifyBeforePayment().then((isIntegrityVerified) {
      print('💳 [Wallet Controller] Post-payment integrity verification result: $isIntegrityVerified');
      PlayIntegrityUtils.logIntegrityCheck('Razorpay Payment Success', isIntegrityVerified);
      
      if (!isIntegrityVerified) {
        print('💳 [Wallet Controller] ⚠️ Payment success processing with integrity warning');
        // Still process the payment but log the warning
      }
  ...
  void handleExternalWaller(ExternalWalletResponse response) {
    print('💳 [Wallet Controller] Razorpay external wallet response received');
    print('💳 [Wallet Controller] Wallet Name: ${response.walletName}');
    
    print('💳 [Wallet Controller] Verifying device integrity for external wallet...');
    // Verify device integrity for external wallet
    PlayIntegrityUtils.verifyBeforePayment().then((isIntegrityVerified) {
      print('💳 [Wallet Controller] External wallet integrity verification result: $isIntegrityVerified');
      PlayIntegrityUtils.logIntegrityCheck('Razorpay External Wallet', isIntegrityVerified);
      
      if (!isIntegrityVerified) {
        print('💳 [Wallet Controller] ⚠️ External wallet processing with integrity warning');
        // Log the integrity issue but still process the external wallet
      }
  ...
  void handlePaymentError(PaymentFailureResponse response) {
    print('💳 [Wallet Controller] Razorpay payment error received');
    print('💳 [Wallet Controller] Error Code: ${response.code}');
    print('💳 [Wallet Controller] Error Message: ${response.message}');
    
    print('💳 [Wallet Controller] Verifying device integrity after payment error...');
    // Verify device integrity after payment error
    PlayIntegrityUtils.verifyBeforePayment().then((isIntegrityVerified) {
      print('💳 [Wallet Controller] Post-error integrity verification result: $isIntegrityVerified');
      PlayIntegrityUtils.logIntegrityCheck('Razorpay Payment Error', isIntegrityVerified);
      
      if (!isIntegrityVerified) {
        print('💳 [Wallet Controller] ⚠️ Payment error occurred with integrity warning');
        // Log the integrity issue but still show the payment error
      }
